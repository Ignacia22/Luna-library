...
exports.default = {
    getBooks: async (req, res) => {
        try {
            ...
            const result = await bookServices_1.default.getBooks(req.query);
            res.status(200).json(result);
        }
        catch (error) { // 🆕 Tipar error
    ...
    getBook: async (req, res) => {
        try {
            ...
            const book = await bookServices_1.default.getBook(id);
            if (!book) {
    ...
    createBooks: async (req, res) => {
        try {
            ...
            const newBook = await bookServices_1.default.createBooks(body);
            res.status(201).json({
    ...
    deleteBook: async (req, res) => {
        try {
            ...
            const result = await bookServices_1.default.deleteBook(id);
            if (!result) {
                res.status(404).json({ message: "Libro no encontrado" });
                return;
            }
